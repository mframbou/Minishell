...
static char *interpret_one_dollar_var_2(char *src, int res_size, char *env_name, char *env_value);
...
/*
	...
	- If it is but not entirely (eg. test_+-+-) => "cut it" so it will become "test_"
		(just like real bash)
	- Then retrieve the associated value in the env (if any, if not "$FOO" becomes "") and :
	
	(Part 2)
	- re-allocate the string
		to add the new value (basically we replace our "$FOO" with "bar") (strlen returns 0 if str is NULL)
	This is done by copying the first part of our string, then the "bar" value (again if any), then the second part of the string
	- env_name = ($)FOO
	- env_value = bar
	- "echo $FOO"
	         ^ ^---- end index
...
static char *interpret_one_dollar_var(char *str, int i)
{
	...
	total_res_size = (sizeof(char) * (ft_strlen(str) - (ft_strlen(env_name) + 1) + ft_strlen(env_value) + 1));
	return (interpret_one_dollar_var_2(str, total_res_size, env_name, env_value));
}
